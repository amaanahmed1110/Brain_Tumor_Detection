...
yes_tumor_img = os.listdir(image_dir+'yes/')
...
for i, image_name in enumerate (no_tumor_img):
    if(image_name.split('.')[1] == 'jpg'):
        image = cv2.imread(f'{image_dir}no/{image_name}')
...
for i, image_name in enumerate (yes_tumor_img):
    if(image_name.split('.')[1] == 'jpg'):
        image = cv2.imread(f'{image_dir}yes/{image_name}')
        image = Image.fromarray(image, 'RGB')
        image=image.resize((INPUT_SIZE,INPUT_SIZE))
        dataset.append(np.array(image))
        label.append(1)
...
# print(y_train.shape)  #(80%, size, size, channel=RGB)
#
# print(x_test.shape)
# print(y_test.shape)
